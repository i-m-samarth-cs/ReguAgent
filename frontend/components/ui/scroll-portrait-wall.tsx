...
const DEMO_SPEAKERS: Speaker[] = [
  ...
  src: `https://images.unsplash.com/photo-${[
    "1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=256",
    "1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=256",
    "1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=256",
    "1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=256",
    "1438761681033-6461ffad8d80?auto=format&fit=crop&q=80&w=256"
  ][i % 5]}`,
}));
...
export function ScrollPortraitWall({
  ...
  return (
    ...
    >
      ...
      <div
        ref={hintRef}
        className="pointer-events-none absolute left-1/2 top-[60vh] grid -translate-x-1/2 content-start justify-items-center gap-6 text-center z-10"
      >
        <span className="relative max-w-[20ch] text-xs uppercase leading-tight text-muted-foreground after:absolute after:left-1/2 after:top-full after:h-16 after:w-px after:bg-gradient-to-b after:from-transparent after:to-muted-foreground/40 after:content-['']">
          {hint}
        </span>
      </div>
              ...
              return (
                <div key={ci} className="aspect-square flex-1">
                  ...
                  >
                    ...
                    />
                    {showCaptions && (
                      <div className="absolute -bottom-2 left-0 flex w-full translate-y-full justify-between gap-2 text-[10px] uppercase leading-tight text-muted-foreground sm:text-xs px-1">
                        <span className="truncate">{s.name}</span>
                        <span className="shrink-0 hidden sm:inline">({s.role})</span>
                      </div>
                    )}
                  </div>
                </div>

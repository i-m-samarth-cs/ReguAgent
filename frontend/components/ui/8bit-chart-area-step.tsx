    ...
    <div className="retro w-full rounded-none border-[6px] border-foreground bg-background p-4 text-foreground shadow-[8px_8px_0_0_var(--foreground)]">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 text-sm font-bold font-mono">Step Area Chart</h3>
        </div>
        <div className="border-[4px] border-foreground bg-card px-3 py-2 text-right text-[10px] leading-relaxed font-mono">
          <span className="block text-muted-foreground">{active.month}</span>
          <span className="text-foreground font-bold">{active.desktop}</span>
        </div>
      </div>
      ...
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-auto w-full overflow-visible" role="img" aria-label="Step area chart for desktop traffic">
        ...
        {yTicks.map((tick) => {
          ...
          return (
            <g key={tick} className="text-muted-foreground">
              ...
              <text x={PADDING.left - 14} y={y + 4} textAnchor="end" fontSize="10" fill="currentColor" className="font-mono">{tick}</text>
            </g>
          );
        })}

        <path d={areaPath()} fill="var(--chart-1)" className="fill-brand-500" opacity="0.38" />
        <path d={stepPath()} fill="none" stroke="var(--chart-1)" className="stroke-brand-600" strokeWidth="6" strokeLinejoin="miter" strokeLinecap="square" />
        ...
        {chartData.map((item, index) => {
          ...
          return (
            <g key={item.month} onMouseEnter={() => setActiveIndex(index)} onFocus={() => setActiveIndex(index)} tabIndex={0} className="cursor-pointer outline-none">
              ...
              <rect x={p.x - 7} y={p.y - 7} width="14" height="14" fill={isActive ? "var(--foreground)" : "var(--chart-1)"} className={isActive ? "fill-foreground" : "fill-brand-500"} stroke="var(--background)" strokeWidth="3" />
            </g>
          );
        })}

        {xTicks.map((tick) => (
          <text key={tick.label} x={tick.x} y={HEIGHT - 18} textAnchor="middle" fontSize="10" fill="currentColor" className="text-muted-foreground font-mono">{tick.label}</text>
        ))}
        ...
        <g transform={`translate(${Math.min(activePoint.x + 14, WIDTH - 170)} ${Math.max(activePoint.y - 62, 18)})`}>
          ...
          <text x="12" y="18" fontSize="10" fill="currentColor" className="font-mono">{active.month}</text>
          <text x="12" y="34" fontSize="10" fill="currentColor" className="font-mono">Desktop: {active.desktop}</text>
        </g>
      </svg>
    </div>
  );
